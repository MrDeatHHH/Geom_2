...
int* fancyColor(int dx, int dy, int max)
{
	...
	if (H < 0)
		...
		H += 180;
	double C = double(double(abs(dx)) + double(abs(dy))) / double(max);
...
{
	Mat Limage_, Limage;
	Limage_ = imread("view0.png", IMREAD_UNCHANGED);
	cvtColor(Limage_, Limage, COLOR_BGR2GRAY);
	//imshow("Gray image L", Limage);

	Mat Rimage_, Rimage;
	Rimage_ = imread("view1.png", IMREAD_UNCHANGED);
	...
	const int maxD = max(abs(minDx), abs(maxDx)) + max(abs(minDy), abs(maxDy));
